...
public class TextStyle implements ITextStyle {
  ...
  public void apply(StyleData sd, int start, int end) {
    if (bold != null) {
      Arrays.fill(sd.bold, start, end, bold);
    }
    if (italic != null) {
      Arrays.fill(sd.italic, start, end, italic);
    }
    if (foreground != null) {
      Arrays.fill(sd.foreground, start, end, foreground);
    }
    if (background != null) {
      Arrays.fill(sd.background, start, end, background);
    }
  }
